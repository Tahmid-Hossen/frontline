...
import portfolioData from "@/data/projectsData.json"
...
const Portfolio = () => {
  ...
  const categories = ["all", ...new Set(portfolioData?.cards?.flatMap(card => card.category))];

  // Filter cards based on selected category
  const filteredCards = selectedCategory === "all"
    ? portfolioData.cards
    : portfolioData.cards.filter(card => card.category.includes(selectedCategory));
      ...
      <div className="container mx-auto px-4 py-8">
          ...
          <TabsList className={"w-full text-black bg-transparent gap-2.5 justify-center md:justify-start"}>
            {categories.map((category, index) => {
              ...
              const count = category === "all"
                ? portfolioData?.cards?.length // Count all cards when "all" is selected
                : portfolioData?.cards?.filter(card => card?.category?.includes(category))?.length;
        ...
        <AnimatedSection>
          <div
            ...
            <h2
              className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-[52px] xl:leading-[55px] font-normal">{portfolioData.section.title}</h2>
            <p
              className="md:ml-6 mb-6 text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl text-center xl:px-6">{portfolioData.section.subtitle}</p>
